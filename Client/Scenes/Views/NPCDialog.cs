...
namespace Client.Scenes.Views
{
    ...
    public sealed class NPCAccessoryLevelDialog : DXWindow
    {
        ...
        private void CalculateCost()
        {
            ...
            count *= Globals.AccessoryLevelCost;  //add in cost calculation
            CostLabel.ForeColour = count > MapObject.User.Gold ? Color.Red : Color.White;

            CostLabel.Text = count.ToString("#,##0");  //uncomment this line

            if (count <= MapObject.User.Gold)   //add if statement to check players has the gold
